...
def on_motion(event):
    ...
    if x is not None and y is not None:
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        detection_radius = 0.005 * (xlim[1] - xlim[0])  # 0.5% of the current x-axis range
        colors = ['grey'] * len(points)
...
def load_points():
    if os.path.exists("points.txt") and os.path.getsize("points.txt") > 0:
        with open("points.txt", "r") as f:
            for line in f:
                try:
                    x, y, vendor_info = line.strip().split(',', 2)
                    points.append({'x': float(x), 'y': float(y), 'vendor_info': vendor_info})
                except ValueError:
                    print(f"Skipping invalid line in points.txt: {line.strip()}")
...
# Check if points.txt is empty or does not exist before calling load_points
if os.path.exists("points.txt") and os.path.getsize("points.txt") > 0:
    load_points()
